...
function getNumbersFromString(string) {
  return hash(string.replace(/\s/g, '').toLowerCase()).toString().split('').reduce((arr, item) => +item ? arr.concat(+item / 10) : arr, [])
}

function drawLine(level, context, spread, size) {
  if (level > 3) return
  context.beginPath()
  context.moveTo(0, 0)
...
function drawFractal(string, context, size, scale) {
  ...
  context.shadowOffsetX = size / 20
  context.shadowOffsetY = size / 20
